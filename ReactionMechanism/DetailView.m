...
#pragma mark --広告画面が出るビュー
    ...
    UIButton *bunnerBtn = [[UIButton alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height-bannerHeight, self.view.frame.size.width, bannerHeight)];  // ボタンのサイズを指定する
...
- (void)setAdmob{
    ...
    [bannerView_ setCenter:CGPointMake(self.view.frame.size.width/2, self.view.frame.size.height-bannerView_.frame.size.height/2)];
